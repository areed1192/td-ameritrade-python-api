...
class TDSession(TestCase):
    ...
    def test_define_simple_order(self):
        ...
        self.td_order.order_duration(duration=td_enums.DURATION.GOOD_TILL_CANCEL)
        
        self.td_order_leg.order_leg_instruction(instruction=td_enums.ORDER_INSTRUCTIONS.SELL)
        self.td_order_leg.order_leg_price(price=112.50)
        self.td_order_leg.order_leg_quantity(quantity=10)
        self.td_order_leg.order_leg_asset(asset_type=td_enums.ORDER_ASSET_TYPE.EQUITY, symbol='MSFT')

...
for minute_period in valid_minute_periods:
    ...
    print('For MINUTE PERIOD {} you got {} worth of minute candles.'.format(hist_period, candle_count))
...
valid_daily_periodTypes = {
    'month': valid_daily_periods_month,
    'year': valid_daily_periods_year, 
    'ytd': valid_daily_periods_ytd
}
...
for period, period_list in valid_daily_periodTypes.items():
    for period_list_item in period_list:
        ...
        # Grab the candle count.
        candle_count = len(historical_minute['candles'])
        print('For PERIOD TYPE {} with DAILY PERIOD {} you got {} candles.'.format(hist_periodType, hist_period, candle_count))
...
for period, period_list in valid_daily_periodTypes.items():
    for period_list_item in period_list:
        ...
        # Grab the candle count.
        candle_count = len(historical_minute['candles'])
        print('For PERIOD TYPE {} with WEEKLY PERIOD {} you got {} candles.'.format(hist_periodType, hist_period, candle_count))
...
for period, period_list in valid_daily_periodTypes.items():
    for period_list_item in period_list:
        ...
        # Grab the candle count.
        candle_count = len(historical_minute['candles'])
        print('For PERIOD TYPE {} with MONTHLY PERIOD {} you got {} candles.'.format(hist_periodType, hist_period, candle_count))
...
accounts_data_single = TDSession.get_accounts(account = TRADING_ACCOUNT,  fields = ['orders'])
...
transaction_data_multi = TDSession.get_transactions(account = TRADING_ACCOUNT, transaction_type = 'ALL')


# TEST - `get_preferences` endpoint. Should not return an error
preference_data = TDSession.get_preferences(account = TRADING_ACCOUNT)

# TEST - `get_subscription_keys` endpoint. Should not return an error
streamer_keys = TDSession.get_streamer_subscription_keys(accounts = [TRADING_ACCOUNT])
